...
chk_row_starts <- c(1, ceiling(nrow(chk) / 3) + 1,
                    ceiling(nrow(chk) / 3) + ceiling(nrow(chk) / 3) + 1)
chk_row_ends <- c(ceiling(nrow(chk) / 3),
                 (ceiling(nrow(chk) / 3) * 2),
                 nrow(chk))

# split table into three subsets
chk_list <- lapply(seq_along(chk_row_starts),
              function(i) chk[seq(chk_row_starts[i], chk_row_ends[i]), ,
                             drop = FALSE])
...
\\begin{table}[!htbp]
\\vspace{-3.0em}%
\\begin{adjustwidth}{-.5in}{-.2in}
\\begin{center}
", paste(collapse = "",
vapply(chk_list, FUN.VALUE = character(1), function(x) {
  ## create table
